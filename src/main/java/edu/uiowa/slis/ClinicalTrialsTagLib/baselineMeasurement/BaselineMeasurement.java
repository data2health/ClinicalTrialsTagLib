...
public class BaselineMeasurement extends ClinicalTrialsTagLibTagSupport {

	static BaselineMeasurement currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(BaselineMeasurement.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			BaselineMeasurementIterator theBaselineMeasurementIterator = (BaselineMeasurementIterator)findAncestorWithClass(this, BaselineMeasurementIterator.class);

			if (theBaselineMeasurementIterator != null) {
				ID = theBaselineMeasurementIterator.getID();
				seqnum = theBaselineMeasurementIterator.getSeqnum();
				measeqnum = theBaselineMeasurementIterator.getMeaseqnum();
				groupId = theBaselineMeasurementIterator.getGroupId();
			}

			if (theBaselineMeasurementIterator == null && theBaselineCategory == null && groupId == null) {
				// no groupId was provided - the default is to assume that it is a new BaselineMeasurement and to generate a new groupId
				groupId = null;
				insertEntity();
			} else {
				// an iterator or groupId was provided as an attribute - we need to load a BaselineMeasurement from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select value,spread,lower_limit,upper_limit from clinical_trials.baseline_measurement where id = ? and seqnum = ? and measeqnum = ? and group_id = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update clinical_trials.baseline_measurement set value = ?, spread = ?, lower_limit = ?, upper_limit = ? where id = ? and seqnum = ? and measeqnum = ? and group_id = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (upperLimit == null)
				upperLimit = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into clinical_trials.baseline_measurement(id,seqnum,measeqnum,group_id,value,spread,lower_limit,upper_limit) values (?,?,?,?,?,?,?,?)");
			stmt.setInt(1,ID);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,measeqnum);
			stmt.setString(4,groupId);
			stmt.setString(5,value);
			stmt.setString(6,spread);
			stmt.setString(7,lowerLimit);
			stmt.setString(8,upperLimit);
			stmt.executeUpdate();
			stmt.close();
		} catch (SQLException e) {
			log.error("Error: IOException while writing to the user", e);

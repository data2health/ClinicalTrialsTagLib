...
public class OfficialDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    int locseqnum = 0;
	...
	private static final Log log = LogFactory.getLog(OfficialDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.official where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
            ...
            log.error("JDBC error generating Official deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating Official deleter");
        } finally {
            freeConnection();
        }

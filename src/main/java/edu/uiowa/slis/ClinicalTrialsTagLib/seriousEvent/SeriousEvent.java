...
public class SeriousEvent extends ClinicalTrialsTagLibTagSupport {

	static SeriousEvent currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(SeriousEvent.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			SeriousEventIterator theSeriousEventIterator = (SeriousEventIterator)findAncestorWithClass(this, SeriousEventIterator.class);

			if (theSeriousEventIterator != null) {
				ID = theSeriousEventIterator.getID();
				seqnum = theSeriousEventIterator.getSeqnum();
				eveseqnum = theSeriousEventIterator.getEveseqnum();
			}

			if (theSeriousEventIterator == null && theSeriousCategory == null && eveseqnum == 0) {
				// no eveseqnum was provided - the default is to assume that it is a new SeriousEvent and to generate a new eveseqnum
				eveseqnum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or eveseqnum was provided as an attribute - we need to load a SeriousEvent from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select sub_title,description,assessment from clinical_trials.serious_event where id = ? and seqnum = ? and eveseqnum = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update clinical_trials.serious_event set sub_title = ?, description = ?, assessment = ? where id = ? and seqnum = ? and eveseqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (eveseqnum == 0) {
				eveseqnum = Sequence.generateID();
				log.debug("generating new SeriousEvent " + eveseqnum);
			}
			...
			if (assessment == null)
				assessment = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into clinical_trials.serious_event(id,seqnum,eveseqnum,sub_title,description,assessment) values (?,?,?,?,?,?)");
			stmt.setInt(1,ID);
			stmt.setString(2,seqnum);
			stmt.setInt(3,eveseqnum);

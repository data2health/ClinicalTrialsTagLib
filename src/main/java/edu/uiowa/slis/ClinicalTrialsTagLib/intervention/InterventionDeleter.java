...
public class InterventionDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
	...
	private static final Log log = LogFactory.getLog(InterventionDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.intervention where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating Intervention deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating Intervention deleter");
        } finally {
            freeConnection();
        }

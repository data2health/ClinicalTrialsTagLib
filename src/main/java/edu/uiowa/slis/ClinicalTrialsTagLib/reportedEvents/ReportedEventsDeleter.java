...
public class ReportedEventsDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Log log = LogFactory.getLog(ReportedEventsDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.reported_events where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
            ...
            log.error("JDBC error generating ReportedEvents deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating ReportedEvents deleter");
        } finally {
            freeConnection();
        }

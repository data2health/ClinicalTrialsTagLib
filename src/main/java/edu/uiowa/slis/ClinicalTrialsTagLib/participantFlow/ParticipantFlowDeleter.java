...
public class ParticipantFlowDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    String recruitmentDetails = null;
    String preAssignmentDetails = null;
	Vector<ClinicalTrialsTagLibTagSupport> parentEntities = new Vector<ClinicalTrialsTagLibTagSupport>();

	private static final Log log = LogFactory.getLog(ParticipantFlowDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.participant_flow where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
            ...
            log.error("JDBC error generating ParticipantFlow deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating ParticipantFlow deleter");
        } finally {
            freeConnection();
        }

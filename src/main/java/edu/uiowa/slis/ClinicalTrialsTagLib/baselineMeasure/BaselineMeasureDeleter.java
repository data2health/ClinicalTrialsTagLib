...
public class BaselineMeasureDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String title = null;
	...
	private static final Log log = LogFactory.getLog(BaselineMeasureDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.baseline_measure where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating BaselineMeasure deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating BaselineMeasure deleter");
        } finally {
            freeConnection();
        }

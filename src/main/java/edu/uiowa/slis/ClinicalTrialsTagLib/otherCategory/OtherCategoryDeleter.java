...
public class OtherCategoryDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    String seqnum = null;
    String title = null;
	Vector<ClinicalTrialsTagLibTagSupport> parentEntities = new Vector<ClinicalTrialsTagLibTagSupport>();

	private static final Log log = LogFactory.getLog(OtherCategoryDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.other_category where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating OtherCategory deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating OtherCategory deleter");
        } finally {
            freeConnection();
        }

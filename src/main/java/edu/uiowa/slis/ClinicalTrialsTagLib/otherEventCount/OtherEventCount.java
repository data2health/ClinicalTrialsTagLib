...
public class OtherEventCount extends ClinicalTrialsTagLibTagSupport {

	static OtherEventCount currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(OtherEventCount.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			OtherEventCountIterator theOtherEventCountIterator = (OtherEventCountIterator)findAncestorWithClass(this, OtherEventCountIterator.class);

			if (theOtherEventCountIterator != null) {
				ID = theOtherEventCountIterator.getID();
				seqnum = theOtherEventCountIterator.getSeqnum();
				eveseqnum = theOtherEventCountIterator.getEveseqnum();
				groupId = theOtherEventCountIterator.getGroupId();
			}

			if (theOtherEventCountIterator == null && theOtherEvent == null && groupId == null) {
				// no groupId was provided - the default is to assume that it is a new OtherEventCount and to generate a new groupId
				groupId = null;
				insertEntity();
			} else {
				// an iterator or groupId was provided as an attribute - we need to load a OtherEventCount from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select subjects_affected,subjects_at_risk,events from clinical_trials.other_event_count where id = ? and seqnum = ? and eveseqnum = ? and group_id = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update clinical_trials.other_event_count set subjects_affected = ?, subjects_at_risk = ?, events = ? where id = ? and seqnum = ? and eveseqnum = ? and group_id = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (events == null)
				events = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into clinical_trials.other_event_count(id,seqnum,eveseqnum,group_id,subjects_affected,subjects_at_risk,events) values (?,?,?,?,?,?,?)");
			stmt.setInt(1,ID);
			stmt.setString(2,seqnum);
			stmt.setInt(3,eveseqnum);
			stmt.setString(4,groupId);

...
public class OtherEventCountDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    String seqnum = null;
    int eveseqnum = 0;
    String groupId = null;
	...
	private static final Log log = LogFactory.getLog(OtherEventCountDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.other_event_count where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating OtherEventCount deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating OtherEventCount deleter");
        } finally {
            freeConnection();
        }

...
public class StudyPop extends ClinicalTrialsTagLibTagSupport {

	static StudyPop currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(StudyPop.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			StudyPopIterator theStudyPopIterator = (StudyPopIterator)findAncestorWithClass(this, StudyPopIterator.class);

			if (theStudyPopIterator != null) {
				ID = theStudyPopIterator.getID();
				seqnum = theStudyPopIterator.getSeqnum();
				elseqnum = theStudyPopIterator.getElseqnum();
			}

			if (theStudyPopIterator == null && theEligibility == null && elseqnum == 0) {
				// no elseqnum was provided - the default is to assume that it is a new StudyPop and to generate a new elseqnum
				elseqnum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or elseqnum was provided as an attribute - we need to load a StudyPop from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select textblock from clinical_trials.study_pop where id = ? and seqnum = ? and elseqnum = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update clinical_trials.study_pop set textblock = ? where id = ? and seqnum = ? and elseqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (elseqnum == 0) {
				elseqnum = Sequence.generateID();
				log.debug("generating new StudyPop " + elseqnum);
			}

			if (textblock == null)
				textblock = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into clinical_trials.study_pop(id,seqnum,elseqnum,textblock) values (?,?,?,?)");
			stmt.setInt(1,ID);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,elseqnum);

...
public class OtherOutcome extends ClinicalTrialsTagLibTagSupport {

	static OtherOutcome currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(OtherOutcome.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			OtherOutcomeIterator theOtherOutcomeIterator = (OtherOutcomeIterator)findAncestorWithClass(this, OtherOutcomeIterator.class);

			if (theOtherOutcomeIterator != null) {
				ID = theOtherOutcomeIterator.getID();
				seqnum = theOtherOutcomeIterator.getSeqnum();
			}

			if (theOtherOutcomeIterator == null && theClinicalStudy == null && seqnum == 0) {
				// no seqnum was provided - the default is to assume that it is a new OtherOutcome and to generate a new seqnum
				seqnum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or seqnum was provided as an attribute - we need to load a OtherOutcome from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select measure,time_frame,safety_issue,description from clinical_trials.other_outcome where id = ? and seqnum = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update clinical_trials.other_outcome set measure = ?, time_frame = ?, safety_issue = ?, description = ? where id = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (seqnum == 0) {
				seqnum = Sequence.generateID();
				log.debug("generating new OtherOutcome " + seqnum);
			}
			...
			if (description == null)
				description = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into clinical_trials.other_outcome(id,seqnum,measure,time_frame,safety_issue,description) values (?,?,?,?,?,?)");
			stmt.setInt(1,ID);
			stmt.setInt(2,seqnum);

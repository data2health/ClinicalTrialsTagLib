...
public class ResultsCategoryDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    int measeqnum = 0;
    int catseqnum = 0;
	...
	private static final Log log = LogFactory.getLog(ResultsCategoryDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.results_category where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
            ...
            log.error("JDBC error generating ResultsCategory deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating ResultsCategory deleter");
        } finally {
            freeConnection();
        }

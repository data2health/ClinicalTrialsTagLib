...
public class OverallOfficialDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
	...
	private static final Log log = LogFactory.getLog(OverallOfficialDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.overall_official where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating OverallOfficial deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating OverallOfficial deleter");
        } finally {
            freeConnection();
        }

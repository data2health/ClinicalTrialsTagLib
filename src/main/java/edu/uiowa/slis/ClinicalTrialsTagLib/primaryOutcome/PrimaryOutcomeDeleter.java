...
public class PrimaryOutcomeDeleter extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String measure = null;
	...
	private static final Log log = LogFactory.getLog(PrimaryOutcomeDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from clinical_trials.primary_outcome where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating PrimaryOutcome deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating PrimaryOutcome deleter");
        } finally {
            freeConnection();
        }

...
public class ClinicalStudyStartDate extends ClinicalTrialsTagLibTagSupport {
	String type = "DATE";
	String dateStyle = "DEFAULT";
	String timeStyle = "DEFAULT";
	String pattern = null;
	private static final Log log = LogFactory.getLog(ClinicalStudyStartDate.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theClinicalStudy.commitNeeded) {
				...
				if (theClinicalStudy.getStartDate() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(theClinicalStudy.getStartDate());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(theClinicalStudy.getStartDate());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(theClinicalStudy.getStartDate());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(theClinicalStudy.getStartDate());
					}
				}
	...
	public void setStartDate(Date startDate) throws JspTagException {
		try {
			ClinicalStudy theClinicalStudy = (ClinicalStudy)findAncestorWithClass(this, ClinicalStudy.class);
			theClinicalStudy.setStartDate(startDate);
		} catch (Exception e) {
			log.error("Can't find enclosing ClinicalStudy for startDate tag ", e);
			throw new JspTagException("Error: Can't find enclosing ClinicalStudy for startDate tag ");
		}

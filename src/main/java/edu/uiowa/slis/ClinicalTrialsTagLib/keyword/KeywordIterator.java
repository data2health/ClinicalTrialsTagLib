...
public class KeywordIterator extends ClinicalTrialsTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String keyword = null;
	...
	public static String keywordCountByClinicalStudy(String ID) throws JspTagException {
		int count = 0;
		KeywordIterator theIterator = new KeywordIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from clinical_trials.keyword where 1=1"
						+ " and id = ?"
						);
	...
	public static Boolean clinicalStudyHasKeyword(String ID) throws JspTagException {
		return ! keywordCountByClinicalStudy(ID).equals("0");
	}
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT clinical_trials.keyword.id, clinical_trials.keyword.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (ID == 0 ? "" : " and id = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("clinical_trials.keyword");
